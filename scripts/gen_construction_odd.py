...
# Allow running as `python scripts/gen_construction_odd.py ...` from the repo root.
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Generate the explicit odd-m (>=5) decomposition construction.")
    ap.add_argument("m", type=int, help="odd modulus m (>=5)")
    ap.add_argument("--out", type=Path, default=None, help="output path (default: artifacts/constructions/odd_m{m}.json)")
    ap.add_argument("--overwrite", action="store_true", help="overwrite output file if it exists")
    ap.add_argument("--no-verify", action="store_true", help="skip verifier check (not recommended)")
    ...
    if not args.no_verify:
        ...
        if not report.ok:
            print(f"Construction produced invalid decomposition: {report.error}", file=sys.stderr)
            return 3
