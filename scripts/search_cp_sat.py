...
# Allow running as `python scripts/search_cp_sat.py ...` from the repo root.
...
def solve_cp_sat(m: int, *, time_limit_s: float, seed: int, log: bool) -> Decomposition | None:
    if m <= 2:
    ...
    # x[c,u,d] = 1 iff cycle c uses arc u -> heads[u][d].
    x = [[[model.new_bool_var(f"x_c{c}_u{u}_d{d}") for d in range(3)] for u in range(n)] for c in range(3)]

    # Arc partition: each outgoing arc (u,d) assigned to exactly one cycle.
    for u in range(n):
        for d in range(3):
            model.add(sum(x[c][u][d] for c in range(3)) == 1)

    # Hamiltonian cycle per color using AddCircuit.
    for c in range(3):
        arcs = []
        for u in range(n):
            for d in range(3):
    ...
    # Symmetry breaking: fix the cycle labels using the origin vertex (idx 0).
    # At u=0, enforce (cycle0 uses d0), (cycle1 uses d1), (cycle2 uses d2).
    model.add(x[0][0][0] == 1)
    model.add(x[1][0][1] == 1)
    model.add(x[2][0][2] == 1)
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Find a 3-cycle decomposition of G_m using OR-Tools CP-SAT.")
    ap.add_argument("m", type=int, help="modulus m (>2)")
    ap.add_argument("--time-limit-s", type=float, default=60.0, help="CP-SAT wall-clock time limit")
    ap.add_argument("--seed", type=int, default=0, help="CP-SAT random seed (determinism with 1 worker)")
    ap.add_argument("--log", action="store_true", help="Enable CP-SAT search progress logging")
    ap.add_argument("--out", type=Path, default=None, help="Output path (default: artifacts/solutions/cpsat_m{m}.json)")
    ...
    decomp = solve_cp_sat(args.m, time_limit_s=args.time_limit_s, seed=args.seed, log=args.log)
    if decomp is None:
    ...
    if not report.ok:
        print(f"Internal error: solver produced invalid decomposition: {report.error}", file=sys.stderr)
        return 3

    save_decomposition(out_path, decomp)
    print(
        f"FOUND: m={args.m} in {decomp.meta.get('wall_time_s'):.3f}s; "
        f"wrote {out_path}"
    )
    return 0
