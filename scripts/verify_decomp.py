...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("path", help="Path to decomposition JSON")
    ...
    if args.json:
            ...
            json.dumps(
                {
                    ...
                    "cycle_checks": [
                        {
                            "cycle_index": check.cycle_index,
                            "ok": check.ok,
                            "error": check.error,
                        }
                        for check in report.cycle_checks
                    ],
                    "error": report.error,
                    "meta": decomp.meta,
                },
        ...
        print(
            ...
            f"cycles_ok={all(check.ok for check in report.cycle_checks)}"
        )
    else:
        print(f"FAIL: {report.error}", file=sys.stderr)
        for check in report.cycle_checks:
            if not check.ok:
                print(f"  cycle {check.cycle_index}: {check.error}", file=sys.stderr)
        if not report.arc_partition_ok:
            print(
                f"  arc partition violations: {report.arc_partition_violations}/{report.n_vertices}",
                file=sys.stderr,
            )
