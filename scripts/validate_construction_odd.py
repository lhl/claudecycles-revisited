...
# Allow running as `python scripts/validate_construction_odd.py ...` from the repo root.
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Validate the odd-m construction over a range of m.")
    ap.add_argument("--m-max", type=int, default=101, help="Validate all odd m in [5, m-max]")
    ap.add_argument(
        "--m-values",
        type=int,
        nargs="*",
