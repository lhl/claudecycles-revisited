...
def load_decomposition(path: str | Path) -> Decomposition:
    ...
    if "dirs" in data:
        ...
        dirs = [_parse_dirs_field(m, item) for item in dirs_raw]
    ...
    raise KeyError(
        "expected either top-level 'dirs' or 'cycles' with per-cycle 'dirs'"
    )
...
def save_decomposition(path: str | Path, decomp: Decomposition) -> None:
    ...
    for c in range(3):
        ...
        dirs_strs.append("".join(str(int(d)) for d in arr))
